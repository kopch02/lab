...
int main(int argc, char *argv[])
{
    ...
    if (rank == 0){
        MPI_Sendrecv(&a, 1, MPI_INT, 2, 777, &a, 1, MPI_INT, size - 1 - nechet, 777, MPI_COMM_WORLD, &stat);
        cout<<"rank ="<<rank<<" a ="<<a<<endl;
    }
    else if(rank == 1){
        MPI_Sendrecv(&b, 1, MPI_INT, 3, 777, &b, 1, MPI_INT, size - 1 - chet, 777, MPI_COMM_WORLD, &stat);
        cout<<"rank ="<<rank<<" b ="<<b<<endl;
    }
    else if (rank == size - 2){
        MPI_Probe(MPI_ANY_SOURCE,MPI_ANY_TAG,MPI_COMM_WORLD,&stat);
        MPI_Sendrecv(&a, 1, MPI_INT, chet, 777, &a, 1, MPI_INT, stat.MPI_SOURCE, 777, MPI_COMM_WORLD, &stat);
    }
    else if (rank == size - 1){
        MPI_Probe(MPI_ANY_SOURCE,MPI_ANY_TAG,MPI_COMM_WORLD,&stat);
